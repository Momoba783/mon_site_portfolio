...
class Controller
{
    ...
    public function handlerRequest() // permet de savoir ce que l'internaute demande (afficher/modifier/supprimer), action de l'internaute
    {
        ...
        try
        {
            ...
            if($_GET) {
                if(isset($_GET["page"]) && $_GET["page"] == "competences") {
                    $this->renderFront("front_layout.php", "services.php", array(
                        "icons" => $this->selectAll2("competences")
                    ));
                }
                if(isset($_GET["page"]) && $_GET["page"] == "connexion") {

                    if($_POST) {
                        // if(strpos("@", $_POST["email"]) !== false && !empty($_POST['password'])) {
                            // var_dump($this->db->checkUser($_POST['email'], $_POST['password']));
                            if($this->db->checkUser($_POST['email'], $_POST['password'])) {
                                $_SESSION['user'] = $this->db->checkUser($_POST['email'], $_POST['password'])["user"];
                                header("location: ?page=admin");
                            }
                        // } else {
                    ...
                    $this->render("layout.php", "connexion.php", array(
                        "title" => "Connexion"
                    ));
                }
    ...
    public function selectAll2($table) {
        return $this->db->selectAll2($table);
    }
    ...
    public function save($op)
    {
        ...
        $values = ($op == 'update') ? $this->db->select($id) : ''; // si on a envoyé un id dans l'URL, on l'envoi en argument de la méthode select() de entityRepository, cela permettra de selectionner toutes les données de l'employé pour la modification

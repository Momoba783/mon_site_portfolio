...
class EntityRepository
{
    ...
    {
        $q = $this->getDb()->query("SELECT * FROM " . $table); // requete permettant de selectionner toute une table, $this->table : représente dans notre cas la table 'employe'
        $r = $q->fetchAll(\PDO::FETCH_ASSOC);
        return $r;
    }
    ...
    public function checkUser($email, $mdp) {
        $q = $this->getDb()->query("SELECT pseudo, email, avatar, age FROM users WHERE email='$email' AND password='$mdp'");
